...
def main():
    ...
    logging.info(f"Parameters: num_clients={num_clients}, k={k}, rounds={rounds}, alpha={alpha}, device={device}")

    # Load data
    logging.info("Loading MNIST data...")
    client_datasets, test_dataset = load_mnist(num_clients, alpha)
    logging.info(f"Data loaded: {len(client_datasets)} client datasets, {len(test_dataset)} test samples")
    ...
    for round_num in range(rounds):
        ...
        local_models = [train_local(copy.deepcopy(global_model_base), client_datasets[i], device=device) for i in selected]
        global_model_base = fedavg_aggregate(local_models, [client_datasets[i] for i in selected], device)
        ...
        local_models = [train_local(copy.deepcopy(global_model_p1), client_datasets[i], device=device) for i in selected]
        global_model_p1 = fedavg_aggregate(local_models, [client_datasets[i] for i in selected], device)
        ...
        local_models = [train_local(copy.deepcopy(global_model_p2), client_datasets[i], device=device) for i in selected]
        rho_t = get_rho_t(round_num, rounds, 'constant', k)
        logging.info(f"Paper 2: rho_t = {rho_t}")
        global_model_p2 = genfed_aggregate(local_models, [client_datasets[i] for i in selected], test_dataset, rho_t, device)
    ...
    logging.info(f"Final Baseline Accuracy: {baseline_acc[-1]:.4f}, Loss: {baseline_loss[-1]:.4f}")
    logging.info(f"Final Paper 1 Accuracy: {paper1_acc[-1]:.4f}, Loss: {paper1_loss[-1]:.4f}")
    logging.info(f"Final Paper 2 Accuracy: {paper2_acc[-1]:.4f}, Loss: {paper2_loss[-1]:.4f}")
