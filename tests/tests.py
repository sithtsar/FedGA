...
class TestFL(unittest.TestCase):
    ...
    def test_fedavg_aggregate(self):
        local_models = [train_local(copy.deepcopy(self.global_model), self.client_datasets[i]) for i in range(3)]
        aggregated = fedavg_aggregate(local_models, [self.client_datasets[i] for i in range(3)])
        self.assertIsNotNone(aggregated)
    
    def test_ga_selection(self):
        local_accs = [0.5 + 0.1 * i for i in range(self.num_clients)]  # mock
        selected = ga_client_selection(self.num_clients, k=3, pop_size=10, generations=2, local_accs=local_accs)
